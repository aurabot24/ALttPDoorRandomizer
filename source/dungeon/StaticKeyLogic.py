...
def set_static_key_rules(world, player):
    from Rules import forbid_item, set_always_allow
    from source.logic.AccessRule import Has, Primitive, add_rule, and_rule, or_rule, set_rule

    def keys(dungeon, count):
        return Primitive('has_sm_key_strict', dungeon_keys[dungeon], player, count)

    def big_key_in(dungeon, locations):
        return Primitive('item_in_named_locations', dungeon_bigs[dungeon], player, tuple(locations))

    def small_key_at(dungeon, location):
        return Primitive('item_is_at', location, player, dungeon_keys[dungeon])
    ...
    def allow_small(dungeon, location, extra=None):
        ...
        # always_allow is still (state, item); extra is an access rule.
        if world.accessibility[player] != 'locations':
            key = dungeon_keys[dungeon]

            def allow(state, item, key=key, extra=extra):
                return item.name == key and item.player == player and (extra is None or extra(state))

            set_always_allow(loc(location), allow)
        else:
    ...
    allow_small('Thieves Town', "Thieves' Town - Big Chest", Has('Hammer', player))
    ...
    somaria = and_rule(Has('Cane of Somaria', player), keys('Ice Palace', 1))
    ...
    set_turtle_rock_rules(world, player, keys, small_key_at, allow_small, forbid_item, set_rule, loc)

    gt = 'Ganons Tower'
    add_rule(loc('Ganons Tower - Firesnake Room'), or_rule(
        keys(gt, 3), and_rule(or_rule(big_key_in(gt, GT_RANDOMIZER),
                                      small_key_at(gt, 'Ganons Tower - Firesnake Room')), keys(gt, 2))))
    for name in GT_RANDOMIZER:
        add_rule(loc(name), or_rule(keys(gt, 4), and_rule(big_key_in(gt, GT_RANDOMIZER), keys(gt, 3))))
    for name in GT_COMPASS:
        add_rule(loc(name), or_rule(keys(gt, 4), and_rule(big_key_in(gt, GT_COMPASS), keys(gt, 3))))
...
def set_turtle_rock_rules(world, player, keys, small_key_at, allow_small, forbid_item, set_rule, loc):
    from source.logic.AccessRule import Primitive, and_rule, not_rule, or_rule
    ...
    def keys_for_chest_item():
        # 0 if the small key is in this chest, 2 if the big key is, otherwise 4.
        small_here = Primitive('item_is_at', chest, player, dungeon_keys[tr])
        big_here = Primitive('item_is_at', chest, player, dungeon_bigs[tr])
        return or_rule(
            small_here,
            and_rule(big_here, keys(tr, 2)),
            and_rule(not_rule(small_here), not_rule(big_here), keys(tr, 4)),
        )
    ...
    if back:
        set_rule(loc(chest), or_rule(keys(tr, 4), small_key_at(tr, chest)))
        ...
        set_rule(loc(chest), keys_for_chest_item())
        allow_small(tr, chest)
